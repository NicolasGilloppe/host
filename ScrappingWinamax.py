...
def scrapp_winamax(driver, home, away, url):
    ...
    for index, odd in enumerate(odds):
        if odd == home:
            ...
            if ',' not in odds[index + path_dict['HD']]:
                odd_hd, odd_da =  odds[index + path_dict['HD'] +1], odds[index + path_dict['DA'] +1]
            else:
                odd_hd, odd_da = odds[index + path_dict['HD']], odds[index + path_dict['DA']]
    ...
    h, odd_ho15 = liste[0].split('\n')
    a, odd_ao15 = liste[2].split('\n')
...
""" driver = initialize_winamax()
time.sleep(1)
print(scrapp_winamax(driver, 'Criciuma', 'Botafogo', 'https://www.winamax.fr/paris-sportifs/match/48214611'))
driver.quit() """
close_all_chrome_instances()
